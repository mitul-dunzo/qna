...
func SendOtpSms(otp string, number string) error {
	...
	url := fmt.Sprintf("https://%s:%s@api.exotel.com/v1/Accounts/%s/Sms/send.json", exotelApiKey, exotelApiToken, exotelSid)

	req, err := http.NewRequest(http.MethodPost, url, strings.NewReader(requestData.Encode()))
	if err != nil {
